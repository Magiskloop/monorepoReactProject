...
const NavTab: FC<NavProps> = ({ navs, isFirstLevel }) => {
					...
					<NavLink
						...
						className={({ isActive }: NavLinkRenderProps) => {
							return (
								...
								(isActive ||
								(getParentPath() === item.path && isFirstLevel)
...
const Navigation = ({ className, hide }: Props) => {
	return (
		...
		>
			<div className="max-w-6xl max-auto my-0 flex justify-center w-full">
				<div
				...
				>
					<div className="w-full h-14 flex justify-between items-center min-w-max">
						<div className="flex items-center">
							<Logo />
							<NavTab navs={router} isFirstLevel={true}></NavTab>
						</div>
						<Search />
						<NavMenu />
					</div>
